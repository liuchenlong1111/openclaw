...
export function extractMessageContent(data: any): ExtractedMessage {
  ...
  switch (msgtype) {
    ...
    case 'richText': {
      ...
      for (const part of parts) {
        ...
        if (part.type === 'audio' && part.downloadCode) {
          downloadCodes.push(part.downloadCode);
          fileNames.push(part.fileName || 'audio.amr');
        }
        // 处理文件
    ...
    case 'audio': {
      ...
      log?.info?.(`Received audio message, full data: ${JSON.stringify(data, null, 2)}`);
      const downloadCodes: string[] = [];
      const fileNames: string[] = [];
      if (audioDownloadCode) {
        downloadCodes.push(audioDownloadCode);
        fileNames.push(audioFileName);
        log?.info?.(`Found audio downloadCode: ${audioDownloadCode}, fileName: ${audioFileName}`);
      } else {
        log?.warn?.(`No downloadCode found in audio message, will not download file! Checked path: data.content.downloadCode = ${audioDownloadCode}`);
      }
      return { 
