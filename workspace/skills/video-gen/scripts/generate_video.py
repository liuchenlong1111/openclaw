...
ENDPOINT_ID = os.environ.get("VIDEO_ENDPOINT_ID", "")
...
def create_video_task(
    ...
    # Ark platform endpoint format - model = endpoint ID
    if ENDPOINT_ID:
        # Ark platform with native content format
        ...
        content.append({
            "type": "text",
            "text": prompt + f" --duration {duration} --aspect-ratio {aspect_ratio}"
        })
        
        # Handle different modes
        if (mode == "image_to_video" or mode == "image_to_video_base64" or 
            mode == "audio_video_first_frame") and first_frame:
            if first_frame.startswith(("http://", "https://")):
                ...
                if os.path.exists(first_frame):
                    ...
                    content.append({
                        "type": "image",
                        "image": f"data:image/png;base64,{first_frame}"
                    })
        
        if mode == "seedance_lite_reference" and reference_image:
        ...
        # Original Doubao API format
        url = f"{BASE_URL}/contents/generations/tasks"
        # Build content array per new API format
        content = []
        ...
        if mode == "audio_video_first_last_frame" and first_frame and last_frame:
            # Similar handling for both frames
            pass
...
def generate_video(
    ...
    print(f"API response: {json.dumps(task, indent=2)}", file=sys.stderr)
    ...
    # Get video URL from new API structure
    video_url = None
